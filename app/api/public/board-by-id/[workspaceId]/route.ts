...
export async function GET(request: NextRequest, context: RouteContext) {
    ...
    const { data: workspace, error } = await supabaseAdmin
        .from('workspaces')
        .select('id,name,nodes,edges,strokes,arrows,updated_at,share_visibility,allow_public_duplicate')
        .eq('id', workspaceId)
        .maybeSingle();
...
export async function PATCH(request: NextRequest, context: RouteContext) {
    ...
    const { data: workspace, error } = await supabaseAdmin
        .from('workspaces')
        .select('id,share_visibility,allow_public_duplicate')
        .eq('id', workspaceId)
        .maybeSingle();

    if (error) {
        console.error('Public board by id edit lookup failed:', error);
    ...
    if (updateError) {
        console.error('Public board by id edit failed:', updateError);
        return NextResponse.json({ error: 'Failed to update board' }, { status: 500 });
    }
